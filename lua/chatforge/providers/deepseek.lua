...
function M.stream(params, handlers)
  ...
  local function handle_line(line)
    ...
    if line:match("^data:%s*%[DONE%]") then return end
  ...
  local job = vim.fn.jobstart(cmd, {
    ...
    on_exit = function(_, code)
      ...
      if code ~= 0 then
        ...
        handlers.on_error("DeepSeek request failed: " .. msg)
        return
      end
      if #stderr > 0 then
      ...
      if text == "" then
        handlers.on_error("Empty response from DeepSeek.")
        return
      end
  ...
  if job <= 0 then
    handlers.on_error("DeepSeek: could not start curl")
    return nil
  end
...
function M.health(cb)
  ...
  vim.schedule(function() cb(true, "DeepSeek API key configured") end)
end
